        ...
        <div class="accordion" id="accordion1">
            <ul class="list-group list-group-flush">
                {foreach $aCategories as $category}
                    <li class="list-group-item pl-1 bg-light d-flex justify-content-between">
                        {if $category->getChildren()}
                            <button class="btn btn-link btn-arrow collapsed  text-dark" type="button" data-toggle="collapse" data-target="#collapse{$category->getId()}" aria-expanded="true" aria-controls="collapse{$category->getId()}">
                            ...
                            <button class="btn btn-link" type="button" aria-label="Close" data-dismiss="modal" data-category-link data-id="{$category->getId()}" data-title="{$category->getTitle()}" aria-expanded="true">
                                Все
                                {component "bs-icon" classes="ml-2" icon="square:r"}
                            </button>
                        {else}
                            <button class="btn btn-link" type="button" aria-label="Close" data-dismiss="modal" data-category-link data-id="{$category->getId()}" data-title="{$category->getTitle()}" aria-expanded="true">
                                {$category->getTitle()}
